...
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_2015_mview AS
    SELECT  t.timestamp, t.date, t.time,
            s.sunrise,s.sunset
...
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time and sun');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time and sun and weather');
...
-- Join time and sun and weatherdata and WR2
DROP MATERIALIZED VIEW IF EXISTS pv3.pv3_time_sun_weather_wr2_2015_mview CASCADE;
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_weather_wr2_2015_mview AS
    SELECT  t.timestamp AS t_timestamp, t.date, t.time,
            s.sunrise,s.sunset,
            w.g_hor_cmp6,w.g_hor_si,w.g_gen_cmp11,w.g_gen_si,w.ev_beleuchtung,w.v_wind,w.d_wind,w.t_luft,w.h_luft,w.p_luft,w.i_niederschlag,
            wr.*
    ...
    LEFT JOIN   pv3.htw_weatherdata_2015 AS w 
        ON (t.timestamp=w.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr2 AS wr
        ON (t.timestamp=wr.timestamp);

-- Join time and sun and weatherdata and WR3
DROP MATERIALIZED VIEW IF EXISTS pv3.pv3_time_sun_weather_wr3_2015_mview CASCADE;
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_weather_wr3_2015_mview AS
    SELECT  t.timestamp AS t_timestamp, t.date, t.time,
            s.sunrise,s.sunset,
            w.g_hor_cmp6,w.g_hor_si,w.g_gen_cmp11,w.g_gen_si,w.ev_beleuchtung,w.v_wind,w.d_wind,w.t_luft,w.h_luft,w.p_luft,w.i_niederschlag,
            wr.*
    ...
    LEFT JOIN   pv3.htw_weatherdata_2015 AS w 
        ON (t.timestamp=w.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr3 AS wr
        ON (t.timestamp=wr.timestamp);

-- Join time and sun and weatherdata and WR4
DROP MATERIALIZED VIEW IF EXISTS pv3.pv3_time_sun_weather_wr4_2015_mview CASCADE;
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_weather_wr4_2015_mview AS
    SELECT  t.timestamp AS t_timestamp, t.date, t.time,
            s.sunrise,s.sunset,
            w.g_hor_cmp6,w.g_hor_si,w.g_gen_cmp11,w.g_gen_si,w.ev_beleuchtung,w.v_wind,w.d_wind,w.t_luft,w.h_luft,w.p_luft,w.i_niederschlag,
            wr.*
    ...
    LEFT JOIN   pv3.htw_weatherdata_2015 AS w 
        ON (t.timestamp=w.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr4 AS wr
        ON (t.timestamp=wr.timestamp);

-- Join time and sun and weatherdata and WR5
DROP MATERIALIZED VIEW IF EXISTS pv3.pv3_time_sun_weather_wr5_2015_mview CASCADE;
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_weather_wr5_2015_mview AS
    SELECT  t.timestamp AS t_timestamp, t.date, t.time,
            s.sunrise,s.sunset,
            w.g_hor_cmp6,w.g_hor_si,w.g_gen_cmp11,w.g_gen_si,w.ev_beleuchtung,w.v_wind,w.d_wind,w.t_luft,w.h_luft,w.p_luft,w.i_niederschlag,
            wr.*
    ...
    LEFT JOIN   pv3.htw_weatherdata_2015 AS w 
        ON (t.timestamp=w.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr5 AS wr
        ON (t.timestamp=wr.timestamp);

-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_wr1_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time, sun, weather, and wr1');
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_wr2_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time, sun, weather, and wr2');
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_wr3_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time, sun, weather, and wr3');
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_wr4_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time, sun, weather, and wr4');
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_wr5_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join time, sun, weather, and wr5');
...
CREATE MATERIALIZED VIEW         pv3.pv3_time_sun_weather_allwr_2015_mview AS
    ...
    LEFT JOIN   pv3.htw_weatherdata_2015 AS w 
        ON (t.timestamp=w.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr1 AS wr1
        ON (t.timestamp=wr1.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr2 AS wr2
        ON (t.timestamp=wr2.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr3 AS wr3
        ON (t.timestamp=wr3.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr4 AS wr4
        ON (t.timestamp=wr4.timestamp)
    LEFT JOIN   pv3.einleuchtend_wrdata_2015_wr5 AS wr5
        ON (t.timestamp=wr5.timestamp);

-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_time_sun_weather_allwr_2015_mview','htw_pv3_postgresql_06_data_join.sql','Join all data');
