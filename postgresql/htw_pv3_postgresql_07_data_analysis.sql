...
SELECT db_log('PV3','v2','output','pv3','pv3_gaps_weather_view','htw_pv3_postgresql_07_data_analysis.sql','Wetterlücken');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_gaps_count_weather_view','htw_pv3_postgresql_07_data_analysis.sql','Anzahl Wetterlücken');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_gaps_count_wr_view','htw_pv3_postgresql_07_data_analysis.sql','Anzahl WR-Lücken');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v1','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Table for related gaps');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Zusammenhängende Lücken in Wetterdaten');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Zusammenhängende Lücken in WR-Daten');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Zusammenhängende Lücken in Wetterdaten und WR-Daten');
...
INSERT INTO pv3.pv3_related_gaps (dataset,gap_count,date,ts_start,ts_end)
    WITH    gap_data_day(timestamp) AS (
                ...
                WHERE g_hor_si IS NULL
                    AND wr1_wrnref IS NULL 
                    AND (time > sunrise AND time < sunset)),
            group_data_day AS (
                SELECT 
                    timestamp,
                    date,
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Zusammenhängende Lücken in Wetterdaten und WR-Daten am Tag');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Zusammenhängende Lücken am Tag');
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v3','output','pv3','pv3_related_gaps','htw_pv3_postgresql_07_data_analysis.sql','Find gaps');
...
UNION ALL
    SELECT 
        ...
        'SELECT count(*) FROM pv3.pv3_time_sun_weather_allwr_2015_mview WHERE g_hor_si IS NULL' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt 
          FROM pv3.pv3_time_sun_weather_allwr_2015_mview 
          WHERE g_hor_si IS NULL) AS sub

UNION ALL
    SELECT 
        2 AS id,
        'Anzahl Lücken in WR-Daten (gesamt)' AS question,
        'SELECT count(*) FROM pv3.pv3_time_sun_weather_allwr_2015_mview WHERE wr1_wrnref IS NULL' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt 
          FROM pv3.pv3_time_sun_weather_allwr_2015_mview 
          WHERE wr1_wrnref IS NULL) AS sub

UNION ALL
    SELECT 
        3 AS id,
        'Anzahl Lücken in Wetterdaten und WR-Daten' AS question,
        'SELECT  FROM pv3.pv3_time_sun_weather_allwr_2015_mview WHERE g_hor_si IS NULL AND wr1_wrnref IS NULL' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt 
          FROM pv3.pv3_time_sun_weather_allwr_2015_mview 
          WHERE g_hor_si IS NULL
            AND wr1_wrnref IS NULL) AS sub

UNION ALL
    SELECT 
        4 AS id,
        'Anzahl Lücken in Wetterdaten (zusammenhängend)' AS question,
        'SELECT count(*) FROM pv3.pv3_related_gaps WHERE dataset = "weather"' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt
          FROM pv3.pv3_related_gaps
          WHERE dataset = 'weather') AS sub

UNION ALL
    SELECT 
        5 AS id,
        'Anzahl Lücken in WR-Daten (zusammenhängend)' AS question,
        'SELECT count(*) FROM pv3.pv3_related_gaps WHERE dataset = "wr"' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt
          FROM pv3.pv3_related_gaps
          WHERE dataset = 'wr') AS sub

UNION ALL
    SELECT 
        6 AS id,
        'Anzahl Lücken in Wetterdaten am Tag' AS question,
        'SELECT count(*) FROM pv3.pv3_time_sun_weather_allwr_2015_mview WHERE g_hor_si IS NULL AND (time < sunrise OR time > sunset)' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt 
          FROM pv3.pv3_time_sun_weather_allwr_2015_mview
          WHERE g_hor_si IS NULL
            AND (time >= sunrise AND time <= sunset)) AS sub
...
UNION ALL
    SELECT 
        8 AS id,
        'Anzahl Lücken in Wetterdaten und WR-Daten' AS question,
        'SELECT count(*) FROM pv3.pv3_time_sun_weather_allwr_2015_mview WHERE dataset = "weather and wr at daytime"' AS query,
        sub.cnt AS solution
    FROM (SELECT count(*) AS cnt 
          FROM pv3.pv3_time_sun_weather_allwr_2015_mview 
          WHERE g_hor_si IS NULL
            OR wr1_wrnref IS NULL) AS sub
...
-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_data_analysis_mview','htw_pv3_postgresql_07_data_analysis.sql','Answer questions');
...
-- Alle Tage mit Lücken - Wetter
DROP MATERIALIZED VIEW IF EXISTS pv3.pv3_data_analysis_weather_days_mview CASCADE;
CREATE MATERIALIZED VIEW         pv3.pv3_data_analysis_weather_days_mview AS
    SELECT date, SUM(gap_count), during_day
    FROM pv3.pv3_related_gaps
    WHERE dataset = 'weather'
    GROUP BY date, during_day
    ORDER BY date;

-- Database Logging (project,version,io,schema_name,table_name,script_name,comment)
SELECT db_log('PV3','v2','output','pv3','pv3_data_analysis_weather_days_mview','htw_pv3_postgresql_07_data_analysis.sql','Days with gaps');
